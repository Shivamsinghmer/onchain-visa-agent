  ...
  return (
    <div className="flex-shrink-0 w-[260px] md:w-[280px] bg-white border border-gray-100 p-5 rounded-2xl flex flex-col space-y-4 shadow-sm transition-all hover:shadow-md hover:-translate-y-1">
      <div className="w-full flex justify-between items-start">
        ...
        <span className={`text-[10px] font-semibold uppercase tracking-widest px-2.5 py-1 rounded-full ${getBadgeColor(processingTime)}`}>
          {processingTime}
        </span>
      </div>
      <div className="flex-1">
        <h4 className="font-serif font-semibold text-base md:text-lg text-[#0A1628] leading-tight">
          {countryName}<br />
      ...
      <div className="flex justify-between items-center pt-4 border-t border-gray-50">
        <span className="font-semibold text-[#4F6EF7] text-lg">
          {visa.price && visa.price !== 0 && visa.price !== '0' ? `$${visa.price}` : 'Free'}
        </span>
        <div className="flex gap-2">
          <button
            onClick={() => onAction(`Tell me more about visa ${visa.id}`)}
            className="h-10 px-3 bg-gray-50 border border-gray-100 rounded-xl text-[10px] font-semibold uppercase tracking-wider text-[#0A1628] hover:bg-gray-100 transition-colors"
          >
            Details
          </button>
          <button
            onClick={() => onAction(`I want to apply for visa ${visa.id}`)}
            className="h-10 px-4 bg-[#0A1628] text-white rounded-xl text-[10px] font-semibold uppercase tracking-wider hover:scale-105 active:scale-95 transition-all shadow-lg shadow-[#0A1628]/20"
          >
            Apply
          </button>
        </div>
