...
const RightPanel = ({ rightPanelOpen, activeApplications, activeVisas }) => {
  return (
    <aside className={`${rightPanelOpen ? 'w-80' : 'w-0'} bg-white border-l border-gray-100 transition-all duration-300 flex flex-col overflow-hidden shrink-0`}>
      <div className="p-8 overflow-y-auto space-y-10 min-w-[320px]">
        
        <div className="pt-2 flex flex-col items-center text-center">
          <div className="w-20 h-20 rounded-full bg-[#F1F3FF] flex items-center justify-center text-4xl mb-4 border border-indigo-50 shadow-inner">
             🌍
          </div>
          <h3 className="text-lg font-serif font-black text-[#0A1628]">Global Travel Hub</h3>
          <p className="text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.1em] mt-1">Real-time Visa Intelligence</p>
        </div>
              ...
              {activeApplications.map((app, i) => (
                <div key={i} className="p-4 rounded-2xl bg-[#F8F9FF] border border-gray-100 shadow-sm transition-transform hover:scale-[1.02]">
                  <div className="flex justify-between items-start mb-2">
                     <p className="text-xs font-black text-[#0A1628] truncate">{app.visaName}</p>
                  ...
                  <p className="text-[10px] font-bold text-[#6B7280]">
                    ID: {app.applicationId || app.id} • {app.submittedAt ? new Date(app.submittedAt).toLocaleDateString() : new Date().toLocaleDateString()}
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="p-6 rounded-3xl bg-[#0A1628]/5 border border-indigo-50/50 text-center flex flex-col items-center">
             {activeVisas.length > 0 ? (
                <div className="text-left w-full space-y-3">
                  <p className="text-[10px] font-black text-[#4F6EF7] mb-2">DOCUMENTS NEEDED FOR {activeVisas[0].country || activeVisas[0].destination}:</p>
        ...
        <div className="space-y-2 pt-4">
           {['Visa Policy Guide', 'Secure Document Center', 'Payment Methods'].map(link => (
             <button key={link} className="w-full text-left px-5 py-3.5 rounded-2xl border border-gray-100 text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280] hover:bg-[#F8F9FF] hover:border-[#4F6EF7]/20 transition-all flex justify-between items-center group shadow-sm">
               {link}
