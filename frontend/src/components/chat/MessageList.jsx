    ...
    <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 scroll-smooth">
      {messages.map((m, idx) => (
        <div key={idx} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
          {m.role === 'assistant' && (
            <span className="text-[9px] font-black uppercase tracking-widest text-[#6B7280] ml-4 mb-2">OnchainCity AI</span>
          )}
          <div className={`max-w-[85%] md:max-w-[75%] p-5 rounded-3xl text-[15px] font-medium leading-relaxed shadow-sm ${
            m.role === 'user' 
            ...
            {m.visas && m.visas.length > 0 && (
              <div className="flex overflow-x-auto gap-4 mt-6 pb-2 -mx-2 scrollbar-hide">
                {m.visas.map((v, i) => (
                  <VisaCard key={i} visa={v} onAction={handleSend} />
                ))}
              </div>
            )}
      ...
      {isStreaming && (
        <div className="flex flex-col items-start animate-fade-in">
           <span className="text-[9px] font-black uppercase tracking-widest text-[#6B7280] ml-4 mb-2">Advisor Thinking</span>
           <div className="bg-[#F1F3FF] p-5 rounded-3xl rounded-bl-none inline-flex gap-1.5 shadow-sm">
      ...
      {activeToolCall && (
        <div className="flex items-center gap-2 ml-4 text-[10px] font-black uppercase tracking-widest text-[#4F6EF7] animate-pulse">
