...
export async function searchVisas(args) {
    const queryParams = {};
    if (args.q) {
        ...
        if (args.destination)
            queryParams.destination = args.destination;
        if (args.category)
            queryParams.category = args.category;
        if (args.citizenship)
            queryParams.citizenship = args.citizenship;
    }
    const params = new URLSearchParams(queryParams).toString();
    const result = await apiRequest(`/visas/search?${params}`);
    const visas = (result.data || []).map((v) => ({
        ...v,
...
export async function getVisaDetails(visaId) {
...
export async function getVisaPricing(visaId) {
    try {
...
export async function getRequiredDocuments(visaId) {
    try {
